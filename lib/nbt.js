...
var NbtReader = {
  ...
  readTag: function(buffer, data) {
    ...
    } else if (type == TAG.LONGARRAY) {
      d = NbtReader.readLongArray(buffer);
      buffer = d.buffer;
      return {type: TAG.LONGARRAY, name: name, val: d.val, buffer: buffer};
	...
	if(!d){
		...
		let sz = buffer.readUInt16BE(1);
		t.name = buffer.toString("UTF-8", 3, sz);
...
var NbtWriter = {
  ...
  writeTag: function(d, withoutHeader) {
    ...
    } else if (d.type === TAG.LONGARRAY) {
      bufs.push(NbtWriter.writeLongArray(d.val));
